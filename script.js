...
/* =========================================================
   KYU Smart Class Clock — Application Logic
...
function saveTimetable(list) {
  saveJSON(LS.timetablePrefix + state.courseId, list);
}

function ensureIds(list) {
  return list.map((c, i) => ({ ...c, id: c.id || `${state.courseId}-${Date.now()}-${i}` }));
}
...
function formatClock(d, format) {
  let h = d.getHours();
  const m = pad(d.getMinutes());
  const s = pad(d.getSeconds());
  if (format === "24") return `${pad(h)}:${m}:${s}`;
...
/* ---------------------------------------------------------
   ...
   Builds concrete dated class instances over a rolling window
   so "next class" can look past today into next week.
--------------------------------------------------------- */
...
/* ---------------------------------------------------------
   7. TOAST (lightweight, no external deps)
--------------------------------------------------------- */
function toast(msg) {
  ...
  if (!el) {
    ...
    el.id = "kyu-toast";
    el.style.cssText = "position:fixed;left:50%;bottom:96px;transform:translateX(-50%);background:var(--surface);border:1px solid var(--border);color:var(--text);padding:10px 18px;border-radius:999px;font-size:13px;z-index:200;box-shadow:0 8px 20px rgba(0,0,0,.3);transition:opacity .3s;";
...
function applyTheme(mode) {
  ...
  els.themeIcon.textContent = mode === "light" ? "☀" : "🌙";
}
...
function initCourse() {
  ...
  if (saved && COURSES[saved]) {
    ...
    state.courseId = Object.keys(COURSES)[0]; // safe default so app still functions
...
function loadSettings() {
  state.settings = { ...DEFAULT_SETTINGS, ...loadJSON(LS.settings, {}) };
}
...
function syncSettingsUI() {
  ...
  if (stdWarn.includes(s.warningMinutes)) {
    ...
    els.warningCustom.hidden = true;
  } else {
    els.warningSelect.value = "custom";
    els.warningCustom.hidden = false;
  ...
  if (stdDur.includes(s.alarmSeconds)) {
    ...
    els.durationCustom.hidden = true;
  } else {
    els.durationSelect.value = "custom";
    els.durationCustom.hidden = false;
  ...
  els.clockFormatToggle.textContent = s.clockFormat === "24" ? "24h" : "12h";
...
els.clockFormatToggle.addEventListener("click", () => {
  state.settings.clockFormat = state.settings.clockFormat === "24" ? "12" : "24";
  persistSettings();
});
els.soundToggle.addEventListener("change", () => {
  state.settings.soundOn = els.soundToggle.checked;
  persistSettings();
});
...
function placeLabel(c) {
  return c.type === "VIRTUAL" ? "💻 Virtual" : `📍 ${c.venue || "TBA"}`;
}

function renderHome(now, occWindow) {
  const format = state.settings.clockFormat;
  els.clockTime.textContent = formatClock(now, format);
  ...
  if (current) {
    ...
    els.currentTypeBadge.textContent = typeBadge(current.type);
    els.currentTypeBadge.className = "type-badge " + (current.type === "VIRTUAL" ? "virtual" : "physical");
    ...
    if (current.type === "VIRTUAL") {
      els.currentJoin.hidden = false;
      if (current.link) {
        els.currentJoin.textContent = "JOIN CLASS";
        els.currentJoin.href = current.link;
        els.currentJoin.style.pointerEvents = "auto";
        els.currentJoin.style.opacity = "1";
      } else {
        els.currentJoin.textContent = "NO CLASS LINK SET";
        els.currentJoin.removeAttribute("href");
        els.currentJoin.style.pointerEvents = "none";
        els.currentJoin.style.opacity = "0.55";
      }
    } else {
      els.currentJoin.hidden = true;
    }
  } else {
  ...
  // Reset both, then decide which (if any) to show — avoids stale content lingering.
  ...
  if (upcomingOverall) {
    ...
    if (sameDay) {
      // Next class is later today — show the ticket, whether or not one is currently in progress.
      els.nextCard.hidden = false;
      fillNextCard(upcomingOverall, now);
    } else if (!current) {
      // Nothing left today and nothing running now — show the "no more classes today" banner.
      els.statusBanner.hidden = false;
      const dayLabel = isTomorrow(upcomingOverall.startDate, now) ? "Tomorrow" : currentDayName(upcomingOverall.startDate);
      els.statusBanner.innerHTML = `
        <div class="sb-title">✓ NO MORE CLASSES TODAY</div>
        <div class="sb-sub">Next class: ${dayLabel}, ${formatTimeShort(upcomingOverall.start, format)}<br>${escapeHTML(upcomingOverall.name)} · ${upcomingOverall.type === "VIRTUAL" ? "Virtual" : escapeHTML(upcomingOverall.venue || "")}</div>
        <div class="sb-countdown">Starts in ${formatHM(upcomingOverall.startDate - now)}</div>
      `;
    }
    // else: a class is currently in progress and the next one isn't today — the live card is enough.
  } else if (!current) {
    els.statusBanner.hidden = false;
    els.statusBanner.innerHTML = `<div class="sb-title">No classes scheduled</div><div class="sb-sub">Add classes in Settings → Edit timetable.</div>`;
  }
  ...
  if (todays.length === 0) {
    ...
    todays.forEach((o) => {
      const status = o.startDate <= now && now < o.endDate ? "current" : (now >= o.endDate ? "done" : "upcoming");
      els.todayList.appendChild(buildClassItem(o, status, format));
    });
  }
}
...
function fillNextCard(next, now) {
  ...
  els.nextTypeBadge.textContent = typeBadge(next.type);
  els.nextTypeBadge.className = "type-badge " + (next.type === "VIRTUAL" ? "virtual" : "physical");
...
function buildClassItem(o, status, format) {
  const div = document.createElement("div");
  div.className = "class-item" + (status === "current" ? " is-current" : "") + (status === "done" ? " is-done" : "");
  const statusLabel = status === "current" ? "NOW" : (status === "done" ? "✓ DONE" : "UPCOMING");
  const statusClass = status === "current" ? "status-current" : (status === "done" ? "status-done" : "status-upcoming");
  div.innerHTML = `
    <span class="class-item-dot ${o.type === "VIRTUAL" ? "virtual" : "physical"}"></span>
    ...
    <span class="class-item-status ${statusClass}">${statusLabel}</span>
  `;
  return div;
}
...
function renderWeekList() {
  ...
  list.forEach((c) => {
    ...
    div.innerHTML = `
      <span class="class-item-dot ${c.type === "VIRTUAL" ? "virtual" : "physical"}"></span>
      <div class="class-item-body">
...
function loadTriggered() {
  const data = loadJSON(LS.triggered, { ids: [] });
  return new Set(data.ids || []);
}
function saveTriggered(set) {
  // prune: keep only last 500 to avoid unbounded growth
  const arr = Array.from(set);
  saveJSON(LS.triggered, { ids: arr.slice(-500) });
}
...
function checkAlarms(now, occWindow) {
  ...
  for (const o of occWindow) {
    if (o.startDate <= now) continue; // class already started/passed — too late for a "pre-class" warning
...
function fireAlarm(o, now) {
  ...
  showAlarmOverlay({
    lead: "CLASS ALERT",
    headline: `${state.settings.warningMinutes} MINUTE${state.settings.warningMinutes == 1 ? "" : "S"} TO CLASS`,
    code: o.code,
    unit: o.name,
...
function showAlarmOverlay(info) {
  ...
  if (state.settings.soundOn) {
    playAlarmSound(state.settings.alarmSeconds);
  }
...
function renderAlarmsTab() {
  ...
  if (todays.length === 0) {
    ...
    todays.forEach((o) => {
      ...
      const div = document.createElement("div");
      div.className = "class-item" + (done ? " is-done" : "");
      div.innerHTML = `
        <span class="class-item-dot ${o.type === "VIRTUAL" ? "virtual" : "physical"}"></span>
        <div class="class-item-body">
  ...
  if (hist.length === 0) {
    ...
    hist.slice(0, 10).forEach((h) => {
      ...
      const div = document.createElement("div");
      div.className = "class-item is-done";
      div.innerHTML = `
        <span class="class-item-dot physical"></span>
        <div class="class-item-body">
...
function openClassForm(cls) {
  ...
  els.fType.value = cls ? cls.type : "PHYSICAL";
  els.fVenue.value = cls ? (cls.venue || "") : "";
  els.fLink.value = cls ? (cls.link || "") : "";
  els.fLecturer.value = cls ? (cls.lecturer || "") : "";
  toggleFormFieldsByType();
  els.fDelete.hidden = !cls;
  showScreen("screen-class-form");
}
$("class-form-close").addEventListener("click", () => showScreen("screen-editor"));

els.fType.addEventListener("change", toggleFormFieldsByType);
function toggleFormFieldsByType() {
  const isVirtual = els.fType.value === "VIRTUAL";
  els.fVenueWrap.hidden = isVirtual;
  els.fLinkWrap.hidden = !isVirtual;
}
...
els.classForm.addEventListener("submit", (e) => {
  ...
  const payload = {
    ...
    venue: els.fType.value === "VIRTUAL" ? "Virtual" : els.fVenue.value.trim(),
    link: els.fType.value === "VIRTUAL" ? els.fLink.value.trim() : "",
...
function tick() {
  ...
  // Keep the Alarms tab reasonably fresh without re-rendering every second unnecessarily
